...
%% 最优个体
    % best_Temp_gate_num= 1000;       % 最优解 临时口使用数量   越少越好  优先级最大
   % best_Progress_time = 10000000; % 最优解 乘客 过程时间    越少越好      次
    % best_Gate_num  = 1000;          % 最优解 固定口 使用数量  越少越好     最低
    best_Score = -100000;           % 得分
...
% 迭代多次 选最好的
%{
best_Temp_gate_num_init = 1000;      % 最优解 临时口使用数量   越少越好  优先级最大
% best_Progress_time_init = 10000000;   % 最优解 乘客 过程时间              次
best_Gate_num_init  = 1000;          % 最优解 固定口 使用数量  越少越好   最低  
max_itr = 10000;
for i=1:max_itr
    i
    % 输入：航次表 登机口表 机票表 乘客时间表
    % 输出：一个解 临时口使用数量 过程时间 固定登机口使用数量
    [Solution, Temp_gate_num_init, Progress_time_init, Gate_num_init] = one_init2(Flight_table, Gate_table,Ticket_table,Progress_time_table); %求解1次
    ...
    if((Temp_gate_num_init < best_Temp_gate_num_init )||...
      ((Temp_gate_num_init == best_Temp_gate_num_init ) && ( Gate_num_init <  best_Gate_num_init))||...
...
%% 交叉 后 使用变异来将不可行解变成可行解
   ...
   disp(str1);
    ...
    NP = one_crossover_mutation(NP, Flight_table, Gate_table, C_weight);
...
%% 变异  ======================================================================
    ...
    for m =1:V_num
     %[NP(:,:,V_NP(m)), Temp_gate_num_init, Progress_time_init, Gate_num_init] = two_variation( NP(:,:,V_NP(m)), Flight_table, Gate_table,Ticket_table,Progress_time_table); %求解1次
     [NP(:,:,V_NP(m))] = one_variation( NP(:,:,V_NP(m)), Flight_table, Gate_table); %求解1次
    end
...
%% 选择  ==============================================================================================
    ...
    disp(str1)   
    % 产生 适应度值
    NP_OBJ = zeros(I_NP,2);%存储每个 个体 的适应度值 (即 临时口使用数量 和  固定口使用数量 决定的得分 ) 和 在种群中的id
    for k = 1:I_NP
        [T, P, G] = one_score(NP(:,:,k), Flight_table, Gate_table, Ticket_table, Progress_time_table);
        NP_OBJ(k,1) = T * -100 + G;   
        NP_OBJ(k,2) = k;% 在种群中的id
    end
    
    % 排序
    NP_OBJ = sortrows(NP_OBJ,-1);%按照第一列 降序排列 
    
    % 选择最好的个体  得分高的
    if( NP_OBJ(1,1) > best_Score )
        best_Solution      = NP(:,:,NP_OBJ(1,2));     % 解
        best_Score = NP_OBJ(1,1);             % 最少的 临时口 使用数量
    end
...
%% 产生新的个体 =======================================================================================
    ...
    died_id = NP_OBJ(I_NP - died_num + 1 : I_NP, 2);% 淘汰后面不好的个体

    str1 = sprintf('begin  generate %d new individuals', died_num);
    disp(str1)  
    
    % 产生新的
    best_Temp_gate_num_new = 1000;      % 最优解 临时口使用数量   越少越好  优先级最大
    % best_Progress_time_new = 10000000; % 最优解 乘客 过程时间              次
    best_Gate_free_num_new  = 1000;    % 最优解 固定口 使用数量  越少越好   最低 
    ...
    for l = 1: died_num
        ...
        for i=1:max_itr
            % 输入：航次表 登机口表 机票表 乘客时间表
            % 输出：一个解 临时口使用数量 过程时间 固定登机口使用数量
            [Solution, Temp_gate_num_new, Progress_time_new, Gate_num_new] = one_init2(Flight_table, Gate_table,Ticket_table,Progress_time_table); %求解1次
            % Solution_W = one_init3(Flight_W, Gate_W); %求解1次
            % Score_W_init = Temp_gate_num_W * -100 + Gate_free_num_W;  %该个体得分

            if((Temp_gate_num_new < best_Temp_gate_num_new )||...
              ((Temp_gate_num_new == best_Temp_gate_num_new ) && ( Gate_num_new <  best_Gate_num_new)))
                %    临时口更少                   或  临时口 使用数量 相同,且 空闲口多
                best_Solution_new       = Solution;
                best_Temp_gate_num_new = Temp_gate_num_new; % 最少的 临时口 使用数量
                % best_Progress_time_new = Progress_time_new; % 最少的 过程时间
