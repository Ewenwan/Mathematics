%%{ 
    function [Solu,Temp_gate_num, Gate_free_num] = one_init2(Flight_table, Gate_table)
        ...
        Solu = zeros(Flight_nums, Gate_nums+1+1); % 初始化解 gate_num + 临时 + 可变异否
        ...
        for i = 1:Flight_nums % 为每个航班选择登机口
           ...
           Solu(i,index) = 1; % 选择该登机口
           Gate_time(index) = Flight_table(i, 2) + 45; % 更新登机口 空闲时间
           
        end
        
        Temp_gate_num = sum(Solu(:,Gate_nums+1));  % 临时口 使用数量   少
        Gate_free_num = 0;
        for j=1:Gate_nums
                if sum(Solu(:,j))==0 % 未占用
