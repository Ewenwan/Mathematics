...
% 单点交叉  k之后的基因全部 交换
function [grop_cros] = two_crossover_mutation(grop_soure, Flight_table, Gate_table, cros_weight)
    ...
    for i = 1 : 2 : grop_nums% 前后两个父代 交叉产生子代，再从交叉点之后检查，不合适的基因，使用变异产出合适的基于
       ...
       % 检查 不可行解，用 变异 产生 可行解 ======================================== 
           grop_cros(:,:,i) = two_crossover_check(grop_cros(:,:,i), Flight_table, Gate_table, cros_point);
           grop_cros(:,:,i+1) = two_crossover_check(grop_cros(:,:,i), Flight_table, Gate_table, cros_point);
       end
