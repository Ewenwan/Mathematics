...
for i=1:max_itr
    ...
    [Solution, Temp_gate_num_init, Progress_time_init, Gate_num_init] = two_init2(Flight_table, Gate_table,Ticket_table,Progress_time_table); %求解1次
...
%% 交叉 后 使用变异来将不可行解变成可行解
   ...
   disp(str1);
    ...
    NP = two_crossover_mutation(NP, Flight_table, Gate_table, C_weight);
...
%% 变异  ======================================================================
    ...
    for m =1:V_num
     ...
     [NP(:,:,V_NP(m))] = two_variation( NP(:,:,V_NP(m)), Flight_table, Gate_table); %求解1次
    end
...
%% 选择  ==============================================================================================
    ...
    for k = 1:I_NP
        [T, P, G] = two_score(NP(:,:,k), Flight_table, Gate_table, Ticket_table, Progress_time_table);
        NP_OBJ(k,1:3) = [T, P, G];   
    ...
    if((NP_OBJ(1,1) < best_Temp_gate_num )||...
      ((NP_OBJ(1,1) == best_Temp_gate_num ) && ( NP_OBJ(1,2) <  best_Progress_time))||...
      ((NP_OBJ(1,1) == best_Temp_gate_num ) && (  NP_OBJ(1,2) == best_Progress_time) && ( NP_OBJ(1,3) < best_Gate_num))) 
        %    临时口更少                   或  临时口 使用数量 相同,且 空闲口多
        best_Solution      = NP(:,:,NP_OBJ(1,4));     % 解
        best_Temp_gate_num = NP_OBJ(1,1);             % 最少的 临时口 使用数量
        best_Progress_time = NP_OBJ(1,2);             % 最少的 过程时间
...
%% 产生新的个体 =======================================================================================
    ...
    for l = 1: died_num
        ...
        for i=1:max_itr
            % 输入：航次表 登机口表 机票表 乘客时间表
            % 输出：一个解 临时口使用数量 过程时间 固定登机口使用数量
            [Solution, Temp_gate_num_new, Progress_time_new, Gate_num_new] = two_init2(Flight_table, Gate_table,Ticket_table,Progress_time_table); %求解1次
            % Solution_W = one_init3(Flight_W, Gate_W); %求解1次
            % Score_W_init = Temp_gate_num_W * -100 + Gate_free_num_W;  %该个体得分

            if((Temp_gate_num_new < best_Temp_gate_num_new )||...
              ((Temp_gate_num_new == best_Temp_gate_num_new ) && ( Progress_time_new <  best_Progress_time_new))||...
              ((Temp_gate_num_new == best_Temp_gate_num_new ) && ( Progress_time_new == best_Progress_time_new) && ( Gate_num_new < best_Gate_num_new))) 
                %    临时口更少                   或  临时口 使用数量 相同,且 空闲口多
