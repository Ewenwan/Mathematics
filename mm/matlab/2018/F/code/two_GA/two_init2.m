function [Solu, Temp_gate_num, Progress_time, Gate_used_num] = two_init2(Flight_table, Gate_table, Ticket_table, Progress_time_table)
   %% 输入：Flight_table 航次表 
    % 航班表 303行 6列：  
    %{
   ...
   %         Gate_num      固定登机口使用数量
    ...
    %% 计算解的适应度值 
      [Temp_gate_num, Progress_time, Gate_used_num] = two_score(Solu, Flight_table, Gate_table, Ticket_table, Progress_time_table); 
%{     
%% Temp_gate_num 临时口使用数量 
