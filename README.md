...
## 10.1 线性系统的状态差分方程    
> **系统预测状态  = 转换矩阵A * 系统上次状态  + 转换矩阵B * 系统输入  + 噪声w()
![](pic/41.png)
...
## 10.2 看一个具体的匀加速运动的实例
> 有一个匀加速运动的小车，它受到的合力为 ft = m*a, a = ft/m, 

> 由匀加速运动的位移和速度公式 xt = xt-1 + vt-1*t + 1/2 * a * t^2 ，

> 能得到由 t-1 到 t 时刻的位移变化公式.

> 对位移求导，得到 速度变化公式 vt = vt-1 + a*t.

![](https://github.com/Ewenwan/Mathematics/blob/master/pic/42.png)

      1. 位移x = 位移x + （速度v + 加速度* 时间/2）* 时间 ； 
...
![](pic/45.png)
      ...
      3. xK是 系统状态的预测值, 大小为 [n, 1]
          H * xk -----> [m, n] * [n, 1] ---> [m, 1]
      4. vk是测量过程噪声(实际使用其协方差(多维)/方差 R)，大小为[m, 1]。
...
#### 10.3.2.1 思考
      ...
      其中Q为系统噪声w的协方差矩阵, R为测量噪声变量v的协方差矩阵。
![](pic/47.png)
...
> 系统估计值Xk = 系统状态方程预测值xk + 增益系数K * (真实测量值Zk - 测量值预测值zk)
      ...
      1.理论预测（先验 Xk）有了，测量值（后验 Zk）也有了，
          那怎么根据这两者得到最优的估计值呢？
      2. 首先想到的就是加权(增益系数K )，或者称之为反馈。
      由一般的反馈思想, 设定值Zk 反馈值zk 做差 Zk - zk 之后 用比例系数K 放大，在加上原来预测值
      得到估计值：
![](pic/49.png)

### 10.4.1 增益系数(反馈加权值，比例系数) K 的求解  
